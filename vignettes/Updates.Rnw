...
\pkg{mosaic} package into a separate package (\pkg{mosaicData}) and that this \emph{not} 
be a dependency of \pkg{mosaic} or of \pkg{fastR}.  So some of the 
data sets used in the book will require loading the new \pkg{mosaicData} package.

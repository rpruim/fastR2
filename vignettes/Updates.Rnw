...
%\VignettePackage{fastR2}
%\VignetteKeywords{mosaic, fastR2, vignettes, resources}
...
\pkg{fastR2} package have been moved to the \pkg{mosaic} package; some of these
...
<<>>=
qdata( ~ Sepal.Length, 0.5, data=iris )
median( ~ Sepal.Length, data=iris )
